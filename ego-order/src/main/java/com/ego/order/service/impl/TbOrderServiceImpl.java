...
public class TbOrderServiceImpl implements TbOrderService {
   ...
   private JedisDao jedisDaoImpl;
    ...
    @Override
    public List<TbItemChild> showOrderCart(List<Long> id, HttpServletRequest request) {
        ...
        for (TbItemChild child : list) {
            for (Long aLong : id) {
                if (child.getId().longValue()==aLong.longValue()){
                    //判断购买量是否大于等于库存
                    TbItem tbItem = tbItemDubboServiceImpl.selById(aLong);
     ...
     Map user =((LinkedHashMap)er.getData());
        ...
        try {
            ...
            if (index>0){
                ...
                for (TbItemChild itemChild : listNew) {
                    listCart.remove(itemChild);
                }
